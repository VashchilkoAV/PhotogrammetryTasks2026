...
TEST (CeresSolver, HelloWorld1) {
    ...
    // Потому что мы вычисляем функцию в CostFunctor1 не в виде 0.5*(10-x)^2, а в виде 10 - x => f'=-1, а класс jet позволяет вычислять табличные производные, а не разностные.
    // Судя по всему ceres выводит якобиан не общего лосса, а конкретно нашей функции потерь: dL/dx = dL/dF * dF/dx,
    // где L -- TrivialLoss, F -- CostFunctor1, y = L(F(x, r))
...
TEST (CeresSolver, HelloWorld2) {
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // ошибка была в функции невязки параболоида -- по сути исходной функцией невязки мы тянули точку к (x_1, y_1, 0) -- эта точка принадлежит параболоиду, но не принадлежит прямой
    }
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    if (outliers_fraction > 0.0 && !use_huber) {
        threshold *= 100.0; // ослабляем порог если есть выбросы и мы к ним не устойчивы (не робастны за счет loss-функции (функции потерь) Huber-а)
    }
    double scale = ideal_line[2] / line_params[2];
    double normed_line_params[3] = {line_params[0] * scale, line_params[1] * scale, line_params[2] * scale};
    std::cout << "Normed line: (a=" << normed_line_params[0] << ", b=" << normed_line_params[1] << ", c=" << normed_line_params[2] << ")" << std::endl;
    
    for (int d = 0; d < 3; ++d) {
        ASSERT_NEAR(normed_line_params[d], ideal_line[d], threshold);
        ...
        // по сути скейл зашит в отношении param_ideal / param_our, домножаем на это выражение все координаты => но приходится увеличивать трешолд. 
        // Далее у нас есть степень свободы в выборе коэффициента, на который будем нормировать
        // Эмперически лучше всего подошел коэффициент c, что возможно объяснимо тем, что он обладает наибольшим модулем
        ...
        // задание прямой двумя параметрами 
        // 1) cos(a) * x + sin(a) * y = b -- требует нормировки;
        // 2) 0.5 * x + b * y + c = 0 -- не требует нормировки, но необходимо знать один из коэффициентов идеальной прямой.
    }
    ...
    ASSERT_GT(inliers_fraction, 0.99 * (1. - outliers_fraction)); // 8 раскоментируйте, почему эта проверка падает? как поправить?
    // 0.99 * max_possible_inliers_fraction = 0.99 * (1. - outliers_fraction)
