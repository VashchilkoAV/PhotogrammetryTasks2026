                                                    ...
                                                    std::vector<cv::DMatch> &filtered_matches)
{
    ...
    for (const auto match: matches) {
        if (match.size() > 1 && match[0].distance / match[1].distance < 0.5f) {
                                                   ...
                                                   std::vector<cv::DMatch> &filtered_matches)
{
    ...
    std::shared_ptr<cv::flann::IndexParams> index_params = flannKdTreeIndexParams(4); // 1
    std::shared_ptr<cv::flann::SearchParams> search_params = flannKsTreeSearchParams(32); // n_matches
    ...
    for (size_t i = 0; i < n_matches; i++) {
        std::set<int> set2train;
        size_t n_consistent_matches = 0;
        for (size_t j = 0; j < total_neighbours; j++) {
            if (distances2_train.at<float>(i, j) <= radius2_train) {
                set2train.insert(indices_train.at<int>(i, j));
            }
        }
        for (size_t j = 0; j < total_neighbours; j++) {
            if (distances2_query.at<float>(i, j) <= radius2_query && set2train.count(indices_query.at<int>(i, j))) {
                n_consistent_matches++;
            } 
        }

        if (n_consistent_matches >= consistent_matches) {
